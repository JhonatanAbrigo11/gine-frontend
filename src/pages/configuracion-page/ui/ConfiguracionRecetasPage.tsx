...
export function ConfiguracionRecetasPage() {
  return (
      ...
      <div className="flex w-full min-w-0 flex-1 flex-col px-6 py-10 lg:px-8">
        <header className="mb-8 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mb-2 font-display text-4xl font-bold tracking-tight text-clinical-900">
              Configuración de <span className="text-primary-700">Recetas Médicas</span>
            </h1>
            <p className="max-w-lg text-sm font-medium text-clinical-800/60">
              Personalice la firma digital, el registro de la ACESS y los parámetros de emisión por defecto para agilizar la entrega de recetas a sus pacientes.
            </p>
          </div>
        </header>
        ...
        <div className="min-h-0 flex-1">
          <div className="glass-card flex min-h-0 w-full flex-col overflow-hidden rounded-[2rem] border border-white bg-white/40 p-6 sm:p-8">
            <ClinicalRecipesSectionEditor baseId="sidebar-config-recipes" />
          </div>
        </div>
      </div>
    </div>
